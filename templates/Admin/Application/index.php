 ...
 * @var iterable<\App\Model\Entity\Application> $application
 */
	...
	<div class="table-responsive">
		...
		?>
               <tr>
                    ...
                    <th><?= $this->Paginator->sort('group_leader_name') ?></th>
                    <th><?= $this->Paginator->sort('date_interview') ?></th>
                    <th><?= $this->Paginator->sort('method') ?></th>
                    <th><?= $this->Paginator->sort('approval_status','status') ?></th>
                    <th class="actions"><?= __('Actions') ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($application as $applicationEntity): ?>
                <tr>
				<td><?php echo $counter++ ?></td>
                    <td><?= ($applicationEntity->user->fullname)?> <br />
						<?= ($applicationEntity->program->name) ?> <br />
                    	<?=($applicationEntity->faculty->name) ?>  
					</td>
                    <td><?= h($applicationEntity->company_name) ?> <br />
                    	...
                    	<?= h($applicationEntity->company_state) ?>
					</td>

                    <td><?= h($applicationEntity->group_leader_name) ?> <br />
						<?= h($applicationEntity->group_leader_phone) ?> <br />
						<?= h($applicationEntity->group_leader_email) ?> <br />
						<?= $this->Number->format($applicationEntity->Team_member) ?>
					</td>

                    <td><?= date('M d,y',strtotime($applicationEntity->date_interview)) ?></td>
                    <td><?= h($applicationEntity->method) ?></td>
                    <td>
					...
					<td class="actions text-center">
						<div class="btn-group shadow" role="group" aria-label="Basic example">
							<?= $this->Html->link(__('<i class="far fa-folder-open"></i>'), ['action' => 'view', $applicationEntity->id], ['class' => 'btn btn-outline-primary btn-xs', 'escapeTitle' => false]) ?>
							<?= $this->Html->link(__('<i class="fa-regular fa-pen-to-square"></i>'), ['action' => 'edit', $applicationEntity->id], ['class' => 'btn btn-outline-warning btn-xs', 'escapeTitle' => false]) ?>
							<?php $this->Form->setTemplates([
								'confirmJs' => 'addToModal("{{formName}}"); return false;'
							]); ?>
							<?= $this->Form->postLink(
								__('<i class="fa-regular fa-trash-can"></i>'),
								['action' => 'delete', $applicationEntity->id],
								[
									'confirm' => __('Are you sure you want to delete Application: "{0}"?', $applicationEntity->id),
									'title' => __('Delete'),
			...
			<div class="row pb-3">
				<div class="col-md-4">
				  <div class="stat_card card-1 bg-body-tertiary">
					<h3><?php echo $total_application; ?></h3>
					<p>Total Application</p>
				  </div>
				</div>
				<div class="col-md-4">
				  <div class="stat_card card-2 bg-body-tertiary">
					<h3><?php echo $total_application_active; ?></h3>
					<p>Active Application</p>
				  </div>
				</div>
				<div class="col-md-4">
				  <div class="stat_card card-3 bg-body-tertiary">
					<h3><?php echo $total_application_archived; ?></h3>
					<p>Archived Application</p>
				  </div>
				</div>
			</div>
...
<div class="row">
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
			<div class="card-title mb-0">Application (Monthly)</div>
...
const monthly = new Chart(ctx, {
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
		<div class="card-title mb-0">Application by Status</div>
			<div class="tricolor_line mb-3"></div>
		<div class="chart-container" style="position: relative;">
...
const status = new Chart(ctx_2, {
    ...
    data: {
        ...
        datasets: [{
            label: '# of Application(s)',
			data: [<?= json_encode($total_application_active); ?>, <?= json_encode($total_application_disabled); ?>, <?= json_encode($total_application_archived); ?>],
            backgroundColor: [
...
<div class="card bg-body-tertiary border-0 shadow mb-4">
	<div class="card-body">
		<div class="card-title mb-0">Search</div>
			<div class="tricolor_line mb-3"></div>
			<?php echo $this->Form->create(null, ['valueSources' => 'query', 'url' => ['controller' => 'Application','action' => 'index']]); ?>
				<fieldset>
					<div class="mb-1"><?php echo $this->Form->control('id',['required' => false]); ?></div>
					<div class="mb-1"> <?php echo $this->Form->control('users_id', ['options' => $users, 
                        'empty' => '---select users---',
                        'class' => 'form-control']); ?></div>
					<div class="mb-1"><?php echo $this->Form->control('faculty_id', ['options' => $faculties, 
                        'empty' => 'select faculty',
                        'class' => 'form-control']); ?></div>
					<div class="mb-1"> <?php echo $this->Form->control('program_id', ['options' => $programs, 
