...
<table width='100%'>
        ...
        <div class="justify">
            ...
            <br /><br />
             3. &nbsp;&nbsp;&nbsp;&nbsp; Di samping itu, kami memohon jasa baik pihak Tuan/puan untuk membenarkan kami merakam sesi temubual tersebut bagi tujuan pembelajaran berterusan.  Dengan ini kami sertakan maklumat untuk pihak puan hubungi <?php echo ($applicationEntity->user->fullname)?> (<?php echo ($applicationEntity->group_leader_email) ?> dan <?php echo ($applicationEntity->group_leader_phone)?>)
              ...
              }elseif ($applicationEntity->approval_status==0){
    ...
    <?php echo $this->Form->create($applicationEntity);?>
    <?php echo $this->Form->control('approval_status',['options'=>['1'=>'approval','2'=>'Rejected'],'class'=>'form-select','empty'=>'select Action']); ?>
    
    <div class="text-end">
    <?php if($applicationEntity->approval_status==1){ ?>
        <?php echo $this->Html->link(('Download PDF'),['action'=>'Pdf',$applicationEntity->id],['class'=>'btn btn-sm btn-outline-primary','escapeTitle'=>false]) ?> <?php
    }?>
        <?php echo $this->Form->button(('Reset'),['type'=>'reset','class'=>'btn btn-outline-warning']); ?>
        <?php echo $this->Form->button(('Submit'),['type'=>'submit','class'=>'btn btn-outline-primary']); ?> 
    </div>
